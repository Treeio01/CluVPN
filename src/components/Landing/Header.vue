<template>
  ...
  >
    ...
    <div class="gap-4 items-center 1025:flex hidden">
      ...
      >
        <span
          class="text-white font-Inter font-medium text-sm tracking-0 leading-[20px]"
        >
          Поддержка
        </span>
      </button>
...
<!-- важно: чтобы absolute меню позиционировалось от этого блока -->
    <button
      @click="openMenu"
      class="py-1.5 px-1.5 mobile:px-[9px] 1025:hidden flex cursor-pointer"
      :aria-expanded="isOpen"
      aria-controls="mobile-menu"
    >
      <img
        src="../../assets/img/menu.svg"
        class="max-w-[32px] max-h-[26px] mobile:max-w-[49px] mobile:max-h-[34px]"
        alt="Открыть меню"
      />
    </button>
    ...
    >
      ...
      >
        <button
          class="cursor-pointer absolute right-[30px] top-[20px]"
          @click="closeMenu"
