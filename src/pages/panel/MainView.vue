<script setup>
import LayoutPanel from "../../components/Panel/LayoutPanel.vue";
import { h, ref } from "vue";
import Questions from "../../components/Panel/Questions.vue";
import Tariffs from "../../components/Panel/Tariffs.vue";
import { fetchUser } from "../../services/user-service.js";
import QuestionBlock from "../../components/Panel/QuestionBlock.vue";

const user = fetchUser();

const tariffs = ref([
  {
    time: "6 месяцев",
    sale: 20,
    price: 200,
    timeAtPrice: "в месяц",
    popular: true,
  },
  {
    time: "3 месяца",
    sale: 15,
    price: 220,
    timeAtPrice: "в месяц",
    popular: false,
  },
  {
    time: "1 месяц",
    sale: 0,
    price: 250,
    timeAtPrice: "в месяц",
    popular: false,
  },
]);

const questions = ref([
  {
    title: "Почему не работает VPN?",
    answer: "ответ1",
    open: false,
  },
  {
    title: "Умный VPN - что это?",
    answer: "ответ2",
    open: false,
  },
  {
    title: "Есть ли ограничения по устройствам?",
    answer: "ответ3",
    open: false,
  },
]);
</script>

<template>
  <LayoutPanel>
    <template #content>
      <div class="flex flex-col gap-[31px] 1441:gap-10 w-full max-w-[1044px]">
        <div
          class="flex md:flex-row flex-col w-full gap-[33px] bg-panel-blue md:rounded-[70px] rounded-[40px] border border-[#5E5E5E]/46"
        >
          <div
            class="flex flex-col items-center w-full max-w-full md:max-w-[510px] gap-[20px] 1441:gap-[30px] md:rounded-[70px] rounded-[40px] border border-[#5E5E5E]/46 py-[25px] md:py-[38px] px-[23px] sm:px-[33px] md:px-[43px]"
          >
            <span
              class="text-white font-Golos font-semibold text-[27px] leading-[130%] tracking-[-0.02em]"
            >
              Ваши подписки
            </span>
            <div
              :class="
                user.isActiveSub ? 'py-[5px] px-[30px]' : 'py-[42px] px-[49px]'
              "
              class="flex bg-primary w-full pb-[40px] items-center rounded-[40px] flex-col gap-[10px]"
            >
              <svg
                v-show="!user.isActiveSub"
                width="48"
                height="47"
                viewBox="0 0 48 47"
                fill="none"
                xmlns="http://www.w3.org/2000/svg"
                xmlns:xlink="http://www.w3.org/1999/xlink"
              >
                <rect
                  x="0.5"
                  width="47"
                  height="47"
                  fill="url(#pattern0_66_2789)"
                />
                <defs>
                  <pattern
                    id="pattern0_66_2789"
                    patternContentUnits="objectBoundingBox"
                    width="1"
                    height="1"
                  >
                    <use
                      xlink:href="#image0_66_2789"
                      transform="scale(0.00195312)"
                    />
                  </pattern>
                  <image
                    id="image0_66_2789"
                    width="512"
                    height="512"
                    preserveAspectRatio="none"
                    xlink:href="data:image/png;base64,"
                  />
                </defs>
              </svg>
              <div class="flex p-2.5" v-show="user.isActiveSub">
                <span
                  class="text-white font-Golos font-medium text-[27px] leading-[120%] tracking-[-0.02em]"
                >
                  Активная подписка
                </span>
              </div>

              <ul v-show="user.isActiveSub" class="flex flex-col gap-2 w-full">
                <li class="flex items-center gap-2.5">
                  <span
                    class="text-white font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    ID:
                  </span>
                  <span
                    class="text-[#D2D2D2] font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    {{ user.subscription.id }}
                  </span>
                </li>
                <li class="flex items-center gap-2.5">
                  <span
                    class="text-white font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    Ключ:
                  </span>
                  <span
                    class="text-[#D2D2D2] font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    {{ user.subscription.key }}
                  </span>
                </li>
                <li class="flex items-center gap-2.5">
                  <span
                    class="text-white font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    Оплачена до:
                  </span>
                  <span
                    class="text-[#D2D2D2] font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    {{ user.subscription.expireTime }}
                  </span>
                </li>
                <li class="flex items-center gap-2.5">
                  <span
                    class="text-white font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    Статус:
                  </span>
                  <span
                    class="text-[#D2D2D2] font-Golos font-medium text-[21px] leading-[120%] tracking-[-0.02em]"
                  >
                    {{ user.subscription.status }}
                  </span>
                </li>
              </ul>
              <span
                v-show="!user.isActiveSub"
                class="text-white font-Golos text-[27px] font-medium text-center"
              >
                К сожалению, у вас нет активных подписок
              </span>
            </div>
            <RouterLink
              to="/sub"
              class="bg-primary flex justify-center hover:opacity-75 transition-opacity ease-in-out duration-300 cursor-pointer rounded-[25px] p-4.5 w-full"
            >
              <span
                class="text-white font-Golos font-semibold text-[21px] leading-[130%] tracking-[-0.02em]"
              >
                Попробовать бесплатно
              </span>
            </RouterLink>
          </div>

          <div
            class="flex flex-col relative gap-[9px] w-full py-[25px] pr-[20px] pl-[20px] md:pr-[38px]"
          >
            <svg
              class="absolute right-[38px] top-[25px] md:block hidden"
              width="52"
              height="52"
              viewBox="0 0 52 52"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M40.1697 8.92667L28.2531 4.46334C27.0181 4.00834 25.0031 4.00834 23.7681 4.46334L11.8514 8.92667C9.55474 9.79334 7.69141 12.48 7.69141 14.9283V32.4783C7.69141 34.2333 8.83974 36.5517 10.2481 37.5917L22.1647 46.4967C24.2664 48.0783 27.7114 48.0783 29.8131 46.4967L41.7297 37.5917C43.1381 36.53 44.2864 34.2333 44.2864 32.4783V14.9283C44.3081 12.48 42.4447 9.79334 40.1697 8.92667ZM31.8064 30.2683C31.4814 30.5933 31.0697 30.745 30.6581 30.745C30.2464 30.745 29.8347 30.5933 29.5097 30.2683L26.0647 26.8233L22.5114 30.3767C22.1864 30.7017 21.7747 30.8533 21.3631 30.8533C20.9514 30.8533 20.5397 30.7017 20.2147 30.3767C19.5864 29.7483 19.5864 28.7083 20.2147 28.08L23.7681 24.5267L20.3014 21.06C19.6731 20.4317 19.6731 19.3917 20.3014 18.7633C20.9297 18.135 21.9697 18.135 22.5981 18.7633L26.0431 22.2083L29.4014 18.85C30.0297 18.2217 31.0697 18.2217 31.6981 18.85C32.3264 19.4783 32.3264 20.5183 31.6981 21.1467L28.3397 24.505L31.7847 27.95C32.4347 28.6 32.4347 29.6183 31.8064 30.2683Z"
                fill="white"
              />
            </svg>
            <div class="flex p-2.5">
              <span
                class="text-white font-Golos md:text-left text-center text-[22px] max-w-full md:max-w-[401px] leading-[130%]"
              >
                Защитите себя и своих близких и получите свободный доступ ко
                всем заблокированным сервисам
              </span>
            </div>

            <Tariffs :tariffs="tariffs" />

            <div class="flex p-2.5">
              <RouterLink
                to="/sub"
                class="text-white hover:opacity-75 transition-opacity ease-in-out duration-300 font-Golos text-[17px] leading-normal tracking-[-0.02em]"
              >
                Не устроили цены? Другие тарифы
              </RouterLink>
            </div>
          </div>
        </div>
        <div class="flex md:flex-row flex-col gap-[45px]">
          <div
            class="flex items-center bg-primary/60 md:rounded-[70px] rounded-[40px] py-[25px] gap-[14px] flex-col w-full max-w-full md:max-w-[371px]"
          >
            <span
              class="flex py-[10px] font-Golos font-semibold text-xl md:text-[27px] text-white"
            >
              Наши преимущества
            </span>
            <div
              class="flex gap-[25px] flex-col w-full bg-panel-blue/70 py-[43px] px-[42px]"
            >
              <span
                class="text-white font-Golos text-xl leading-[100%] tracking-[-0.02em]"
              >
                ✓ Безлимитный трафик
              </span>
              <span
                class="text-white font-Golos text-xl leading-[100%] tracking-[-0.02em]"
              >
                ✓ Техподдержка
              </span>
              <span
                class="text-white font-Golos text-xl leading-[100%] tracking-[-0.02em]"
              >
                ✓ Блокировка рекламы
              </span>
              <span
                class="text-white font-Golos text-xl leading-[100%] tracking-[-0.02em]"
              >
                ✓ 1 подписка для всех устройств
              </span>
            </div>
            <RouterLink
              to="/sub"
              class="py-[10px] flex w-full px-[42px] text-[#B2B2B2] leading-[100%] font-Golos text-lg"
            >
              Больше преимуществ <br class="md:block hidden" />
              На нашем сайте →
            </RouterLink>
          </div>
          <QuestionBlock :questions="questions" title="Частые вопросы" />
        </div>
      </div>
    </template>
  </LayoutPanel>
</template>

<style scoped></style>
